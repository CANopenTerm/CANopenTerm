...
**Returns:** Decoded output as a string or `nil` if no matching entry was found.
...
```lua
oscilloscope_buffer_create (min_value, max_value)
```
...
if osc_id >= 0 then
  print("Oscilloscope buffer created: " .. osc_id)
else
...
-- Add sample values
for i = 1, 50 do
  oscilloscope_buffer_push(osc_id, math.random(0, 100))
end
