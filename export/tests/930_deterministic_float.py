...
def assertEqual(a, b):
    ...
    if isinstance(a, float) and isinstance(b, float):
        ...
        if max_val == 0:
            ...
            rel_tol = abs_diff / max_val
            if rel_tol < 1e-14:  # Allow 1e-14 relative difference
                return
    print(f'{a} != {b} ({a-b})')
...
assertEqual(math.inf, math.inf)
assert math.nan != math.nan

# test ceil
assertEqual(math.ceil(math.pi), 4.0)
assertEqual(math.ceil(-math.e), -2.0)
# Note: pocketpy does not properly handle ceil(inf), so this test is skipped
